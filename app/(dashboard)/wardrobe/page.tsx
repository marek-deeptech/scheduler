...
function MemberPickerModal({
  teamId, currentMemberIds, onClose, onSaved,
}: {
  teamId: string
...
export default function WardrobePage() {
  ...
  async function fetchBase() {
    setLoading(true)
    const { data: teamData } = await supabase.from('teams').select('id').eq('name', 'Wardrobe').single()
    const tid = teamData?.id ?? null
    setTeamId(tid)
    if (!tid) { setLoading(false); return }

    const [{ data: memberData }, { data: prodData }] = await Promise.all([
      supabase.from('artists').select('id, name, email, phone, role, status, avatar_url').eq('team_id', tid).order('name'),
  ...
  async function fetchWeek() {
    ...
    const [{ data: evData }, { data: avData }] = await Promise.all([
      ...
      supabase.from('availabilities').select('*')
        .in('artist_id', memberIds)
        .lte('start_time', weekEnd.toISOString()).gte('end_time', weekStart.toISOString()),
    ])
  ...
  async function removeMember(memberId: string) {
    if (!confirm('Usunąć tę osobę z zespołu Kostiumów?')) return
  ...
  function getEventsForDay(memberId: string, day: Date): EventSlot[] {
    ...
    return weekEvents.filter(e => toDateStr(new Date(e.start_time)) === ds && e.artistIds.includes(memberId))
  }
  ...
  function getAvailForDay(memberId: string, day: Date): Avail | null {
    ...
    return availabilities.find(a => {
      ...
      return dayStr >= a.start_time.slice(0, 10) && dayStr <= a.end_time.slice(0, 10)
    }) ?? null
  }

  if (loading) return <div className="max-w-6xl mx-auto"><p className="text-gray-400 text-sm">Ładowanie...</p></div>
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-12">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Kostiumy i Garderoba</h2>
      ...
      <section>
        ...
        {members.length === 0 ? (
          <div className="text-center py-12 bg-white border border-dashed border-gray-200 rounded-2xl text-gray-400">
            ...
            <p className="text-sm mt-1">Użyj przycisków powyżej, aby dodać osoby do garderoby</p>
          </div>
        ) : (
      ...
      <section>
        ...
        {productions.length === 0 ? (
          <p className="text-sm text-gray-400 italic bg-white border border-dashed border-gray-200 rounded-2xl py-8 text-center">
            Żaden członek garderoby nie jest jeszcze przypisany do produkcji.<br />
            ...
            {productions.map(p => (
              <div key={p.id} className="bg-white border border-gray-200 rounded-2xl p-5 flex flex-col gap-3 hover:shadow-sm transition-shadow">
                ...
                <div className="border-t border-gray-50 pt-3">
                  <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-2">Garderoba</p>
