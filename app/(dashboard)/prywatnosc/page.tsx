...
// Klauzula informacyjna (art. 13 RODO). Dane administratora uzupełniane per org
// (lib/rodo-admin). Miejsca w [nawiasach] wymagają uzupełnienia dla danej org.

export default function PrivacyNoticePage() {
  const { org } = useOrg()
  const a = rodoAdminFor(org?.slug)
  const admin = a?.name ?? org?.name ?? '[nazwa teatru — administrator danych]'
  ...
  return (
    <div className="max-w-3xl mx-auto pb-24">
      ...
      <div className="rounded-xl px-4 py-3 mb-5 text-xs" style={{ background: '#fffbeb', border: '1px solid #fde68a', color: '#92400e' }}>
        {a
          ? <>⚠ Dane administratora uzupełniono z oficjalnych źródeł ({a.updated}). Treść merytoryczną zatwierdza IOD / prawnik administratora.</>
      ...
      <P>Administratorem Twoich danych osobowych jest <b>{admin}</b>, {a?.address ?? '[adres]'}
        {a?.nip ? <>, NIP {a.nip}</> : ', [NIP]'}{a?.regon ? <>, REGON {a.regon}</> : null}
        {a?.register ? <> (wpisany do rejestru: {a.register})</> : null}.</P>
      <P>Kontakt: {a?.email ?? '[e-mail]'}{a?.phone ? <>, tel. {a.phone}</> : ', [telefon]'}.</P>
      <P>Inspektor Ochrony Danych (IOD): {a?.iod ?? '[e-mail IOD]'}
        {a ? <>, lub pisemnie na adres siedziby administratora.</> : '.'}</P>
      ...
      <P>Niektórzy dostawcy usług IT mogą przetwarzać dane poza Europejskim Obszarem Gospodarczym. W takim wypadku odbywa się to na podstawie odpowiednich zabezpieczeń (standardowe klauzule umowne / decyzja o adekwatności / EU-US Data Privacy Framework). Szczegóły udostępnia administrator.</P>
      {a?.noEogTransfer && (
        <P>Administrator deklaruje brak przekazywania danych poza EOG — dobór dostawców usług dla tej aplikacji musi to zapewniać (np. regiony UE).</P>
      )}
      ...
      <p className="text-[11px] mt-8" style={{ color: '#b8b0a4' }}>Data ostatniej aktualizacji: {a?.updated ?? '[data]'}. Zatwierdza: administrator / IOD.</p>
    </div>
  )
}
